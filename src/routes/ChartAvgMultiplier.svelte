<script>
	...
	const updateChart = () => {
		...
		['avax', 'bsc', 'eth', 'ftm', 'poly'].forEach((chain) => {
			const avgMultiplier = parseFloat(stats[chain].current.aveMultiplier).toFixed(2);
			plotData.push(avgMultiplier);
		});
	};

	$: if (stats && chart) {
		updateChart();
		unifiedValue = parseFloat(stats.unified.current.aveMultiplier).toFixed(2);
