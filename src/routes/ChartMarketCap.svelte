<script>
	...
	const updateChart = () => {
		...
		['avax', 'bsc', 'eth', 'ftm', 'poly'].forEach((chain) => {
			const mktCap = (parseFloat(stats[chain].current.marketCap) / 1000000).toFixed(2);
			plotData.push(mktCap);
		});
	};

	$: if (stats && chart) {
		updateChart();
		unifiedValue = (parseFloat(stats.unified.current.marketCap) / 1000000).toFixed(2);
		...
		plugins: [
			ChartDataLabels,
			{
				...
				beforeDraw: function (chart, a, b) {
					...
					var text = `Unified: ${unifiedValue}M`,
